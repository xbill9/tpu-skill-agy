...
This repository packages the **`tpu-management` skill** and the **`tpu-devops` MCP server** (a FastMCP app in `server.py`). It is a multi-agent toolkit supporting both **Claude Code** and **Google Antigravity**. Together they:
...
- **Plugin marketplace:** `.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json` make the repo installable via `/plugin marketplace add xbill9/tpu-management-skill`. Validate changes with `claude plugin validate .`
