#!/bin/bash
# Writes a .env for the Gemini CLI / Antigravity ADK integration and points gcloud
# at the saved project. Reads the project ID from ~/project_id.txt (written by
# init.sh) and the Gemini API key from ~/gemini.key (prompting once if missing).
set -euo pipefail
