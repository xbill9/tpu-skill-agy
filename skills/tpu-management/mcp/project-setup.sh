...
install_skill() {
  ...
  if [ "$src" = "$dest" ]; then
    info "skill already at destination ($label): $dest"
    return
  fi
  ...
  rm -rf "$dest"
  cp -r "$src" "$dest"
  info "skill installed ($label): $dest"
}
...
# --- Register for Antigravity (agy format) -------------------------------------------
# Antigravity (agy) picks up global MCP servers from ~/.gemini/antigravity-cli/mcp/<name>/
# where each tool is a separate .json schema file.
register_agy_mcp() {
  local server_name="$1"
  local mcp_dir="$HOME/.gemini/antigravity-cli/mcp/$server_name"
  
  info "Registering MCP server '$server_name' in agy format (schemas) -> $mcp_dir"
  mkdir -p "$mcp_dir"
  
  # Extract tool schemas from server.py and write them as individual JSON files.
  # We use the local server.py to ensure we have the latest tools.
  "$PYTHON_BIN" - "$mcp_dir" <<'EOF'
import asyncio
import json
import os
import sys

# Ensure we can import server.py from the current directory
...
async def dump_schemas(dest_dir):
    try:
        ...
        for t in tools:
            ...
            with open(os.path.join(dest_dir, f"{t.name}.json"), "w") as f:
                json.dump(schema, f, indent=2)
                f.write("\n")
        print(f"  ... wrote {len(tools)} tool schemas to {dest_dir}")
    except Exception as e:
        print(f"  ... error extracting schemas: {e}", file=sys.stderr)

if __name__ == "__main__":
    asyncio.run(dump_schemas(sys.argv[1]))
EOF
}

register_agy_mcp "$SERVER_NAME"
